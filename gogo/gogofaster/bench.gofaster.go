...
var (
	...
	message        [][]byte
	cpuInfo        [][]byte
	basicMemInfos  []GoGoFasterBasicMemInfo
	memInfos       []GoGoFasterMemInfo
	messages       []GoGoFasterMessage
	redditAccounts []GoGoFasterRedditAccount
	cpuInfos       []GoGoFasterCPUInfo
)

func newBench(s string) benchutil.Bench {
	bench := benchutil.NewBench("gogo/protobuf/protoc-gen-gogofaster")
...
func basicMemInfoUnmarshal(b *testing.B) {
	var tmp GoGoFasterBasicMemInfo
	for i := 0; i < b.N; i++ {
		for j := 0; j < shared.Len; j++ {
...
func redditAccountUnmarshal(b *testing.B) {
	var tmp GoGoFasterRedditAccount
	for i := 0; i < b.N; i++ {
		for j := 0; j < shared.Len; j++ {
...
func PrepareBasicMemInfoData(data []shared.ShBasicMemInfo) []GoGoFasterBasicMemInfo {
	tmp := make([]GoGoFasterBasicMemInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoFasterBasicMemInfo{
...
func PrepareMemInfoData(data []shared.ShMemInfo) []GoGoFasterMemInfo {
	tmp := make([]GoGoFasterMemInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoFasterMemInfo{
			MemTotal:          data[i].MemTotal,
			MemFree:           data[i].MemFree,
			MemAvailable:      data[i].MemAvailable,
			Buffers:           data[i].Buffers,
...
func PrepareMessageData(data []shared.ShMessage) []GoGoFasterMessage {
	tmp := make([]GoGoFasterMessage, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoFasterMessage{
			ID:     data[i].ID,
...
func PrepareRedditAccountData(data []shared.ShRedditAccount) []GoGoFasterRedditAccount {
	tmp := make([]GoGoFasterRedditAccount, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoFasterRedditAccount{
			ID:   data[i].ID,
			Name: data[i].Name,
			Kind: data[i].Kind,
			Data: &GoGoFasterAccountData{
...
func PrepareCPUInfoData(data []shared.ShCPUInfo) []GoGoFasterCPUInfo {
	tmp := make([]GoGoFasterCPUInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		inf := GoGoFasterCPUInfo{CPUs: make([]*GoGoFasterCPU, 0, len(data[i].CPUs))}
