...
var (
	...
	message        [][]byte
	cpuInfo        [][]byte
	basicMemInfos  []GoGoSlickBasicMemInfo
	memInfos       []GoGoSlickMemInfo
	messages       []GoGoSlickMessage
	redditAccounts []GoGoSlickRedditAccount
	cpuInfos       []GoGoSlickCPUInfo
)

func newBench(s string) benchutil.Bench {
	bench := benchutil.NewBench("gogo/protobuf/protoc-gen-gogoslick")
...
func redditAccountUnmarshal(b *testing.B) {
	var tmp GoGoSlickRedditAccount
	for i := 0; i < b.N; i++ {
		for j := 0; j < shared.Len; j++ {
...
func PrepareBasicMemInfoData(data []shared.ShBasicMemInfo) []GoGoSlickBasicMemInfo {
	tmp := make([]GoGoSlickBasicMemInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoSlickBasicMemInfo{
...
func PrepareMemInfoData(data []shared.ShMemInfo) []GoGoSlickMemInfo {
	tmp := make([]GoGoSlickMemInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoSlickMemInfo{
			MemTotal:          data[i].MemTotal,
			MemFree:           data[i].MemFree,
			MemAvailable:      data[i].MemAvailable,
			Buffers:           data[i].Buffers,
...
func PrepareMessageData(data []shared.ShMessage) []GoGoSlickMessage {
	tmp := make([]GoGoSlickMessage, 0, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoSlickMessage{
			ID:     data[i].ID,
...
func PrepareRedditAccountData(data []shared.ShRedditAccount) []GoGoSlickRedditAccount {
	tmp := make([]GoGoSlickRedditAccount, len(data))
	for i := 0; i < len(data); i++ {
		tmp = append(tmp, GoGoSlickRedditAccount{
			ID:   data[i].ID,
			Name: data[i].Name,
			Kind: data[i].Kind,
			Data: &GoGoSlickAccountData{
...
func PrepareCPUInfoData(data []shared.ShCPUInfo) []GoGoSlickCPUInfo {
	tmp := make([]GoGoSlickCPUInfo, 0, len(data))
	for i := 0; i < len(data); i++ {
		inf := GoGoSlickCPUInfo{CPUs: make([]*GoGoSlickCPU, 0, len(data[i].CPUs))}
